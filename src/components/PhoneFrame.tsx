...
export function PhoneFrame({ children, hideNav }: { children: ReactNode; hideNav?: boolean }) {
  ...
  return (
    <div className="min-h-screen w-full flex bg-background">
      ...
      {!isAuthPage && isDirector && (
        ...
        >
          ...
          <nav className="flex-1 px-3 space-y-1">
            <SidebarItem to="/dashboard" icon={<LayoutDashboard />} label="Dashboard" collapsed={sidebarCollapsed} active={location.pathname === "/dashboard"} />
            <SidebarItem to="/admin-users" icon={<Users />} label="Gestión de Usuarios" collapsed={sidebarCollapsed} active={location.pathname === "/admin-users"} />
            <SidebarItem to="/messages" icon={<MessageSquare />} label="Mensajería Institucional" collapsed={sidebarCollapsed} active={location.pathname === "/messages"} />
            <SidebarItem to="/library" icon={<Library />} label="Librería Digital" collapsed={sidebarCollapsed} active={location.pathname === "/library"} />
            <SidebarItem to="/ai-lab" icon={<Sparkles />} label="Laboratorio IA" collapsed={sidebarCollapsed} active={location.pathname === "/ai-lab"} />
            <SidebarItem to="/settings" icon={<Settings />} label="Configuración" collapsed={sidebarCollapsed} active={location.pathname === "/settings"} />
          </nav>
      ...
      {/* ÁREA DE CONTENIDO */}
      <main className={`flex-1 relative overflow-hidden ${isAuthPage ? 'flex items-center justify-center' : 'flex flex-col'}`}>
...
function SidebarItem({ to, icon, label, collapsed, active }: { to: string, icon: any, label: string, collapsed: boolean, active: boolean }) {
  return (
    <Link 
      to={to}
