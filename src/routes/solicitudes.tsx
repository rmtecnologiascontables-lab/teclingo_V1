...
export const Route = createFileRoute("/solicitudes")({
  ...
  head: () => ({
    meta: [{ title: "Aprobar Solicitudes · TecLingo" }],
  }),
});
...
function SolicitudesPanel() {
  ...
  const processedSolicitudes = solicitudes.filter(s => s.status !== "pendiente");
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="px-4 pt-4 pb-2 border-b border-zinc-800">
          <div className="flex items-center gap-2 mb-1">
            <Link to="/superadmin" className="text-zinc-400 hover:text-zinc-200">
              <ArrowLeft className="w-4 h-4" />
            </Link>
            <h1 className="text-lg font-bold text-zinc-100">Aprobar Solicitudes</h1>
          </div>
          <p className="text-xs text-zinc-500">{pendingSolicitudes.length} solicitudes pendientes</p>
        </div>
                    ...
                    >
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-1 py-2 bg-green-600 hover:bg-green-500 disabled:bg-zinc-700 rounded text-xs font-medium"
                        >
                        ...
                        <button
                          ...
                          disabled={loadingAction === sol.id}
                          className="flex-1 flex items-center justify-center gap-1 py-2 bg-red-600 hover:bg-red-500 disabled:bg-zinc-700 rounded text-xs font-medium"
                        >
                          {loadingAction === sol.id ? (
                            <Loader2 className="w-3 h-3 animate-spin" />
                          ) : (
            ...
            {processedSolicitudes.length > 0 && (
              <div>
                <h2 className="text-xs font-bold text-zinc-500 uppercase mb-2">
                  Procesadas ({processedSolicitudes.length})
                </h2>
                <div className="space-y-1">
                  {processedSolicitudes.map((sol) => (
                    <div key={sol.id} className="flex items-center justify-between py-2 px-3 bg-zinc-900/40 rounded">
                      <div>
                        <p className="text-xs text-zinc-300">{sol.nombre}</p>
                        <p className="text-xs text-zinc-500">{sol.email}</p>
                      </div>
                      <span className={`text-xs px-2 py-0.5 rounded ${
                        sol.status === "aprobado" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"
                      }`}>
                        {sol.status}
                      </span>
                    </div>
