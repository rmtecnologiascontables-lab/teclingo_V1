...
function RegisterSuccessPage() {
  ...
  const handleDownloadPDF = useCallback(() => {
    ...
    doc.text(`Rol: ${session.role === "director" ? "Director" : session.role === "teacher" ? "Docente" : "Alumno"}`, 20, 65);
    ...
    <PhoneFrame>
      <div className="min-h-screen flex flex-col px-5 pt-10 pb-8">
        ...
        >
          ...
          <h1 className="text-xl font-extrabold text-gradient mt-4">
            ¡Registro exitoso!
          </h1>
          ...
          <div className="relative w-[280px] h-[440px] bg-white rounded-[24px] shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="mt-[60px] px-6 flex flex-col items-center flex-1 text-center">
              ...
              <p className="text-sm font-semibold text-cyan-600 uppercase tracking-widest mt-1">
                {session.role === "director" ? "Director" : session.role === "teacher" ? "Docente" : "Alumno"}
              </p>
              
              <div className="mt-4 w-full text-xs text-gray-600 space-y-1 text-left">
                <p><span className="font-bold">Institución:</span> {session.institutionName || "TecLingo"}</p>
                {session.role === "student" && (
                  <>
                    <p><span className="font-bold">Carrera:</span> {session.carrera}</p>
                    <p><span className="font-bold">Semestre:</span> {session.semestre}</p>
                    <p><span className="font-bold">No. Control:</span> {session.numeroControl}</p>
                  </>
                )}
              </div>
              ...
              <div className="mt-auto mb-6 flex flex-col items-center">
                ...
                <p className="text-[10px] font-mono font-bold text-gray-400 mt-2">{session.app_code}</p>
              </div>
            </div>
          </div>
        </motion.div>
